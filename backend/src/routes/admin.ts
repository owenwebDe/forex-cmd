...
router.get("/dashboard-stats", async (req: AuthRequest, res) => {
  try {
    ...
    const stats = {
      ...
      totalBalance: allAccounts.reduce((sum: number, acc: any) => sum + (acc.balance || 0), 0),
      totalEquity: allAccounts.reduce((sum: number, acc: any) => sum + (acc.equity || 0), 0),
      totalProfit: allAccounts.reduce((sum: number, acc: any) => sum + (acc.profit || 0), 0),
      openPositions: allAccounts.reduce((sum: number, acc: any) => sum + (acc.positions?.length || 0), 0),
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      logger.info("User created by admin:", { userId: newUser.id, adminId: req.user?.id })
...
router.post("/users/:userId/disable", [param("userId").trim().isLength({ min: 1 })], async (req: AuthRequest, res) => {
  try {
...
router.post("/users/:userId/enable", [param("userId").trim().isLength({ min: 1 })], async (req: AuthRequest, res) => {
  try {
    const { userId } = req.params
...
router.get("/mt5-accounts/:loginId", [param("loginId").isNumeric()], async (req: AuthRequest, res) => {
  try {
  ...
  try {
    ...
    for (const account of accounts) {
      try {
        ...
        logger.warn(`Failed to get positions for account ${account.loginId}:`, error)
      }
    }
...
router.post("/close-position", [body("ticket").isNumeric()], async (req: AuthRequest, res) => {
  try {
    const { ticket } = req.body

    const result = await mt5Service.sendCloseTrade({ ticket })

    logger.info("Position closed by admin:", { ticket, adminId: req.user?.id })
    res.json(result)
  } catch (error) {
...
router.post(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      logger.info("Trade opened by admin:", { tradeData, adminId: req.user?.id })
      res.json(result)
    } catch (error) {
